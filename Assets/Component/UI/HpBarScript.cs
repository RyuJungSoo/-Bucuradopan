...
public class HpBarScript : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        m_objectList = new List<Transform>[poolManager.monsterPrefabs.Length]; // 몬스터 오브젝트들의 transform 값들을 저장할 리스트
        m_hpBarList = new List<GameObject>[poolManager.monsterPrefabs.Length]; // hpBar 오브젝트들을 저장할 리스트
        m_MonsterList = new List<MonsterComponent>[poolManager.monsterPrefabs.Length]; // 몬스터 오브젝트들을 저장할 리스트
        m_RectTransform = new List<RectTransform>[poolManager.monsterPrefabs.Length]; // hpBar 오브젝트들의 RectTransform 값들을 저장할 리스트
    ...
    public void AddMonster(int index, GameObject monsterObj) // 새로운 몬스터 오브젝트가 추가될 때
    {
        ...
        t_hpbar.transform.SetAsFirstSibling(); // 몬스터 체력바가 맨 앞으로 오는 것을 막음.
